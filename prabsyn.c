...
static void pr_var(FILE *out, A_var v, int d) 
{
    ...
    switch (v->kind) {
        ...
        case A_fieldVar:
            fprintf(out, "%s\n", "fieldVar(");
            pr_var(out, v->u.field.var, d + 4); fprintf(out, "%s\n", ","); 
            indent(out, d + 4); fprintf(out, "%s)", S_name(v->u.field.sym));
            break;
        case A_subscriptVar:
            fprintf(out, "%s\n", "subscriptVar(");
            pr_var(out, v->u.subscript.var, d + 4); fprintf(out, "%s\n", ","); 
            pr_exp(out, v->u.subscript.exp, d + 4); fprintf(out, "%s", ")");
            break;
        default:
            assert(0); 
    } 
...
void pr_exp(FILE *out, A_exp v, int d) 
{
    ...
    switch (v->kind) {
        ...
        case A_callExp:
            ...
            pr_expList(out, v->u.call.args, d + 4); fprintf(out, ")");
            break;
        case A_opExp:
            fprintf(out, "opExp(\n");
            indent(out, d + 4); pr_oper(out, v->u.op.oper); fprintf(out, ",\n"); 
            pr_exp(out, v->u.op.left, d + 4); fprintf(out, ",\n"); 
            pr_exp(out, v->u.op.right, d + 4); fprintf(out, ")");
            break;
        case A_recordExp:
            fprintf(out, "recordExp(%s,\n", S_name(v->u.record.typ)); 
            pr_efieldList(out, v->u.record.fields, d + 4); fprintf(out, ")");
            break;
        case A_seqExp:
            fprintf(out, "seqExp(\n");
            pr_expList(out, v->u.seq, d + 4); fprintf(out, ")");
            break;
        case A_assignExp:
            fprintf(out, "assignExp(\n");
            pr_var(out, v->u.assign.var, d + 4); fprintf(out, ",\n");
            pr_exp(out, v->u.assign.exp, d + 4); fprintf(out, ")");
            break;
        case A_ifExp:
            fprintf(out, "iffExp(\n");
            pr_exp(out, v->u.iff.test, d + 4); fprintf(out, ",\n");
            pr_exp(out, v->u.iff.then, d + 4);
            if (v->u.iff.elsee) { /* else is optional */
                fprintf(out, ",\n");
                pr_exp(out, v->u.iff.elsee, d + 4);
            }
            fprintf(out, ")");
            break;
        case A_whileExp:
            fprintf(out, "whileExp(\n");
            pr_exp(out, v->u.whilee.test, d + 4); fprintf(out, ",\n");
            pr_exp(out, v->u.whilee.body, d + 4); fprintf(out, ")\n");
            break;
        case A_forExp:
            fprintf(out, "forExp(%s,\n", S_name(v->u.forr.var)); 
            pr_exp(out, v->u.forr.lo, d + 4); fprintf(out, ",\n");
            pr_exp(out, v->u.forr.hi, d + 4); fprintf(out, "%s\n", ",");
            pr_exp(out, v->u.forr.body, d + 4); fprintf(out, ",\n");
            indent(out, d + 4); fprintf(out, "%s", v->u.forr.escape ? "TRUE)" : "FALSE)");
            break;
        case A_breakExp:
        ...
        case A_letExp:
            fprintf(out, "letExp(\n");
            pr_decList(out, v->u.let.decs, d + 4); fprintf(out, ",\n");
            pr_exp(out, v->u.let.body, d + 4); fprintf(out, ")");
            break;
        case A_arrayExp:
            fprintf(out, "arrayExp(%s,\n", S_name(v->u.array.typ));
            pr_exp(out, v->u.array.size, d + 4); fprintf(out, ",\n");
            pr_exp(out, v->u.array.init, d + 4); fprintf(out, ")");
            break;
        default:
            assert(0); 
    } 
...
static void pr_dec(FILE *out, A_dec v, int d) 
{
    ...
    switch (v->kind) {
        case A_functionDec:
            ...
            pr_fundecList(out, v->u.function, d + 4); fprintf(out, ")");
            break;
        case A_varDec:
            ...
            pr_exp(out, v->u.var.init, d + 4); fprintf(out, ",\n");
            indent(out, d + 4); fprintf(out, "%s", v->u.var.escape ? "TRUE)" : "FALSE)");
            break;
        case A_typeDec:
            fprintf(out, "typeDec(\n"); 
            pr_nametyList(out, v->u.type, d + 4); fprintf(out, ")");
            break;
        default:
            assert(0); 
    } 
...
static void pr_ty(FILE *out, A_ty v, int d) 
{
    ...
    switch (v->kind) {
        ...
        case A_recordTy:
            ...
            pr_fieldList(out, v->u.record, d + 4); fprintf(out, ")");
            break;
        case A_arrayTy:
...
static void pr_fieldList(FILE *out, A_fieldList v, int d) 
{
    ...
    if (v) {
        ...
        pr_field(out, v->head, d + 4); fprintf(out, ",\n");
        pr_fieldList(out, v->tail, d + 4); fprintf(out, ")");
    }
...
static void pr_expList(FILE *out, A_expList v, int d) 
{
    ...
    if (v) {
        fprintf(out, "expList(\n"); 
        pr_exp(out, v->head, d + 4); fprintf(out, ",\n");
        pr_expList(out, v->tail, d + 4);
        fprintf(out, ")");
    }
...
static void pr_fundecList(FILE *out, A_fundecList v, int d) 
{
    ...
    if (v) {
        ...
        pr_fundec(out, v->head, d + 4); fprintf(out, ",\n");
        pr_fundecList(out, v->tail, d + 4); fprintf(out, ")");
    }
...
static void pr_decList(FILE *out, A_decList v, int d) 
{
    ...
    if (v) {
        fprintf(out, "decList(\n"); 
        pr_dec(out, v->head, d + 4); fprintf(out, ",\n");
        pr_decList(out, v->tail, d + 4);
        fprintf(out, ")");
    }
...
static void pr_nametyList(FILE *out, A_nametyList v, int d) 
{
    ...
    if (v) {
        ...
        pr_namety(out, v->head, d + 4); fprintf(out, ",\n");
        pr_nametyList(out, v->tail, d + 4); fprintf(out, ")");
    }
...
static void pr_efield(FILE *out, A_efield v, int d) 
{
    ...
    if (v) {
        ...
        pr_exp(out, v->exp, d + 4); fprintf(out, ")");
    }
...
static void pr_efieldList(FILE *out, A_efieldList v, int d) 
{
    ...
    if (v) {
        ...
        pr_efield(out, v->head, d + 4); fprintf(out, ",\n");
